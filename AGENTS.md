...
written to be complete rather than short. (For the design-rationale narrative —
why the architecture is shaped this way, not just what's where — see
[`onboard/docs/introduction.md`](onboard/docs/introduction.md).)
...
| `lock_tracker_fused.*` | **`track::LockTracker` — the CURRENT tracker**, ported from the Kotlin `LockTracker.kt` (validated in `desktop/simtrack.py`, which mirrors both). Cue fusion, anchor+adaptive+keyframe bank, STAPLE histogram cue, ego-motion feed-forward, occlusion-aware adaptation, zoomed-out SEARCHING re-acquire. **OpenCV-free** (plain float arrays via `gray_frame.*`, `crop_filters.*`, `optical_flow.*`, `center_filter.hpp`) so it unit-tests headless. Supersedes `lock_tracker.*`, which the runtime still uses |
| `tools/lockon_rec.cpp` | Field-test recorder: an FC **AUX switch** arms the lock, video is written with the overlay burned in plus a CSV. **READ ONLY on the FC** — links `MspBackend` for telemetry, never calls `sendControl`. Keep it that way |
