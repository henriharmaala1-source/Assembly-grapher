...
#
# MEASURED, over the whole SYNTHETIC battery (eval_lkcoast.py). These clips
# are generated by synth_footage.py, not recorded -- and all ten share one
# background realisation from seed 1234, so they are not ten independent
# samples either:
#
#     clip                    base    with LK
...
#     g_occlusion              79%        90%
#     e_recede                100%       100%
#     MEAN                     70%        77%   <-- SINGLE DRAW, DO NOT QUOTE
#
# THE +7 IS NOT REAL. eval_noisefloor.py perturbs the input in ways that cannot
# matter -- moving the designation box one pixel, adding luma noise below the
# 8-bit quantisation step -- and the mean swings across 18 points, with one clip
# swinging 67. The 77% column above is the luckiest of ten such draws; the mean
# of those draws is 69%. Under a proper paired ensemble (14 draws, same
# perturbations applied to both arms) the coast assist is worth
#
#     +2.35 +/- 0.97 points (t = 2.43, positive in 10 of 14 draws)
#
# So it IS a real win, and it is roughly a third of what was first reported.
#
# What justifies shipping it is not the mean but the per-clip SIGN, which is
# stable across draws and matches the mechanism: f_maneuver better in 12 of 14
# (median 19 -> 37), i_worst better in 12 of 14 (median 14 -> 26), d_pan_shake
# WORSE in 13 of 14 (median 71 -> 69). Flow helps when the appearance model fails
# on a moving target and mildly hurts when the camera shakes.
#
# One earlier claim here was wrong and is withdrawn: "g_occlusion 79% -> 90%".
# Over 14 draws LK wins 6 of 14 there, medians 69 vs 68 -- no effect at all.
#
# Cost: the 18.8-vs-19.1 ms/frame figure was measured with 12 jobs on 4 cores and
# cannot resolve 2%. The conclusion (cost is negligible) is safe on operation
# counts; the measurement was not sound.
...
def ncc_maps(chan, tmpls, tns, g0, g1, stride=STRIDE):
    ...
    per template. Measured over 307 synthetic-battery frames the tracker evaluates ~3641
...
class Tracker:
    ...
    def update(self, frame):
        ...
        if LK_ASSIST and LK_MODE<2 and self.bad>0 and self.lkpts is not None:
            ...
            lkd=self._step(grey)
        # Position BEFORE the prediction step, so mode 2 can replace the
        # extrapolation rather than add to it. See the mode-2 block below.
        ...
        if LK_ASSIST and LK_MODE==2:
            if self.state!='LOCKED' and self.lkpts is not None:
                ...
                lkd=self._step(grey)
                if lkd is not None:
                    # REPLACE the extrapolation, do not add to it. This was a
                    # real defect: the coast branch has already set bcx to
                    # `prev + vx + edx`, and flow measures TOTAL image motion --
                    # target plus camera -- so `bcx += lkd` counted the velocity
                    # step and the camera pan twice. Measured on a target moving
                    # exactly 5.00 px/frame, the first coast step came out at
                    # 10.12 px, and over 8 forced coast frames the box finished
                    # +12.5 px past the target where plain coasting finished
                    # -13.9 px short. At f_maneuver's ~18 px/frame against a 36 px
                    # box that is more than a box width of overshoot injected in
                    # the first three coast frames. It also double-counted ego,
                    # which is the more likely explanation of the consistent
                    # d_pan_shake regression than "flow mildly hurts on shake".
                    self.bcx=self.prex+lkd[0]; self.bcy=self.prey+lkd[1]
