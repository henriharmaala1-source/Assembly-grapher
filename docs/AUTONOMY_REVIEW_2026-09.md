...
- **`flow_odometry_check` passed partly on aliasing -- fixed 2026-09-25.** The synthetic IR point-sampled a texture finer than a pixel at range, and the flow estimator's fixed variance floor (25) had been tuned on that: band-limit the render and it solved 2–4 of 19 frames. The floor is now 4 (`nav-sim/flow_velocity.hpp`), the check solves 19 of 19 on the band-limited render with translation 5–8 % off truth (16–25 % on the old render), and the band-limited render is the default everywhere (`CamParams::irBandLimit = true`).
