...
int menu(const std::string& dir) {
    for (;;) {
        std::printf(
            ...
            "  5  the demo: the aircraft, flying      (demo)\n"
...
int main(int argc, char** argv) {
    ...
    if (cmd == "--help" || cmd == "-h" || cmd == "help") {
        std::printf(
            ...
            "\n"
            "  demo              the showcase: the AIRCRAFT'S OWN autonomy (onboard's\n"
            "                    voxel module and mission, compiled in) flying a\n"
            "                    simulated world -- the flight, what it sees, the\n"
            "                    stereo depth and the map it builds. With --live,\n"
            "                    --replay or --model: four panes -- depth from a real\n"
            "                    camera, its map, and people with a range each.\n"
            "                    --live takes a RealSense; --cuda puts the policy\n"
            "                    and an --detector onnx on the GPU. With no camera\n"
            "                    every pane still runs off the sim. --shot PREFIX\n"
            "                    and --check need no camera and no display.\n"
