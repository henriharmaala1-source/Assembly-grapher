...
void VoxelEnv::reset(const std::string& world, unsigned seed) {
    ...
    if (world == "maze") {
        ...
        g.sizeM = 160.f;
        g.pitchM = wrand(12.f, 16.f);
        g.wallFrac = (world == "hall") ? 0.65f : 0.30f;
        g.ceiling = (world == "hall");
        // TALL ENOUGH THAT OVER IS NOT A ROUTE. The aircraft flies at 4 m and
        // can climb; an obstacle it can top is one it does not have to avoid,
        // and a demo of a planner that never turns is a demo of nothing.
        g.minHM = (world == "hall") ? 10.f : 14.f;
        g.maxHM = (world == "hall") ? 14.f : 24.f;
        float sx = 0, sy = 0, gx = 0, gy = 0;
