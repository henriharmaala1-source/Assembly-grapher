...
// --- MAZE -- the cul-de-sac repeated until it is a routing problem ----------
//
// WHY A MAZE IS A DIFFERENT TEST. genCulDeSac has ONE trap and the escape is
// always behind you; a reactive layer that backs out solves it. A maze has a
// trap at every junction, and the correct move at a dead end is to commit to a
// branch you have already rejected once. That is the first world here that
// cannot be flown on local clearance alone.
//
// It also stresses two things measured elsewhere in this tree and never
// together. UNKNOWN IS NOT FREE: an unexplored corridor and a dead end look
// identical until the far wall is inside marking range, so a planner that
// scores unknown as open will prefer whichever branch it has looked at least.
// And CORRIDOR DEGENERACY: scan_match_check showed that a corridor pins almost
// nothing -- translation along it is unobservable and yaw is below the map's
// angular resolution -- so this is also the honest worst case for any odometry
// that leans on geometry.
//
// GENERATED BY RANDOMISED DEPTH-FIRST SEARCH on a cell grid, which produces a
// PERFECT maze: exactly one path between any two points, no loops. That is the
// hard version deliberately -- a maze with loops lets a wrong turn recover by
// going round, and the failure being tested here is the one where it cannot.
//
...
struct MazeParams {
    ...
    float corridorM = 4.0f;           // clear width of a corridor
    float wallT    = 0.5f;
    float wallH    = 3.0f;            // a ceiling would make it indoor; this is
                                      // open-topped, so climbing out IS possible
                                      // and the climb penalty is under test too
    float tex      = 0.65f;
    bool  ceiling  = false;           // close the top: then it is a real maze
    unsigned seed  = 1;
};
// `startM`/`goalM` receive the world-space centres of the entry and exit cells,
// because a maze whose spawn is not in a corridor is not a maze test.
void genMaze(VoxelWorld& w, const MazeParams& p,
             float* startX = nullptr, float* startY = nullptr,
             float* goalX = nullptr, float* goalY = nullptr);
